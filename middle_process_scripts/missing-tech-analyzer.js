...
function analyzeMissingTechnologies() {
  ...
  // 保存分析结果
  const analysisPath = path.join(projectRoot, 'data', 'missing-technologies-analysis.json');
  const fullAnalysis = {
    ...missingAnalysis,
    missingIds,
...
function generateSupplementaryExtractionPlan(missingAnalysis) {
  ...
  const planPath = path.join(projectRoot, 'data', 'supplementary-extraction-plan.json');
