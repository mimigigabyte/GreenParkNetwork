...
// 所有剩余的技术URL（除了已处理的第一项）
const remainingTechUrls = [
...
// 添加新的技术数据到集合
function addTechData(newTechData) {
  if (Array.isArray(newTechData)) {
    collectedTechData.push(...newTechData);
  } else {
    collectedTechData.push(newTechData);
  }
  console.log(`新增技术数据 ${Array.isArray(newTechData) ? newTechData.length : 1} 项`);
  console.log(`当前总数: ${collectedTechData.length}/98`);
}
