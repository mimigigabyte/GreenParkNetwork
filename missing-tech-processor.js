...
class MissingTechProcessor {
  ...
  generateProcessingReport() {
    ...
    // 保存报告
    const reportPath = path.join(__dirname, 'data', 'missing-tech-processing-report.json');
  ...
  mergeWithMainDatabase() {
    const mainDataPath = path.join(__dirname, 'data', 'improved-69-wipo-wind-technologies.json');
    ...
    // 保存合并后的数据
    const mergedPath = path.join(__dirname, 'data', `complete-${mainData.length + this.windTechs.length}-wipo-wind-technologies.json`);
