...
export function SubcategoryForm({ categoryId, subcategory, onSuccess, onCancel }: SubcategoryFormProps) {
  ...
  const handleNameZhChange = (value: string) => {
    setFormData(prev => ({
      ...
      slug: !subcategory && !prev.slug ? generateSlug(value) : prev.slug
    }))
    // 清除相关错误
    if (errors.name_zh) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="例如：Industrial Energy Saving Technology"
            />
            {errors.name_en && (
          ...
          <div>
            ...
            <input
              ...
              placeholder="例如：industrial-energy-saving"
            />
            {errors.slug && (
              <p className="text-red-500 text-sm mt-1">{errors.slug}</p>
            )}
            <p className="text-gray-500 text-xs mt-1">
              只能包含小写字母、数字和连字符，在当前分类下唯一
            </p>
          </div>
