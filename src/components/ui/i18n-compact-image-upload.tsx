...
interface I18nCompactImageUploadProps {
  value?: string
...
export function I18nCompactImageUpload({
  value,
  onChange,
  bucket = 'images',
  folder = 'uploads',
  ...
  const getText = (key: string) => {
    const translations = {
      zh: {
        selectImage: '请选择图片文件',
        fileSizeError: `文件大小不能超过 ${maxSize}MB`,
        uploadFailed: '上传失败：',
        retry: '请重试',
        uploading: '上传中...',
        uploadedLogo: '已上传Logo',
        edit: '更换',
        delete: '删除',
        support: `支持PNG、JPG，不超过${maxSize}MB`,
        clickUpload: '点击上传'
      },
      en: {
        selectImage: 'Please select an image file',
        fileSizeError: `File size cannot exceed ${maxSize}MB`,
        ...
        edit: 'Edit',
        delete: 'Delete',
        support: `Support PNG, JPG, max ${maxSize}MB`,
        clickUpload: 'Click to Upload'
      }
    }
    return translations[locale as keyof typeof translations]?.[key as keyof typeof translations.zh] || translations.zh[key as keyof typeof translations.zh]
  }

  const defaultPlaceholder = placeholder || getText('clickUpload')
  ...
  const handleFileSelect = async (files: FileList | null) => {
    ...
    try {
      ...
      alert(getText('uploadFailed') + (error instanceof Error ? error.message : getText('retry')))
    } finally {
      setUploading(false)
    }
  }
      ...
      {value ? (
        ...
        <div className="flex items-center space-x-2 w-full h-full">
          ...
          <div className="flex-1 min-w-0">
            <span className="text-sm text-gray-600 truncate block">{getText('uploadedLogo')}</span>
          </div>
        ...
        >
          {uploading ? (
            <div className="flex items-center space-x-2 flex-1">
              ...
              <span className="text-gray-600">{getText('uploading')}</span>
            </div>
          ) : (
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center space-x-2">
                <Upload className="w-4 h-4 text-gray-400" />
                <span className="text-gray-600">{defaultPlaceholder}</span>
              </div>
